...
1) Profitability: ROA
We look at the return on asset of our sample in order to determine differences in profitability between solvent and insolvent firms. First of all we check the distribution. 
```{r}
ggplot(merged_data, aes(x=roa)) +
    geom_histogram(colour="black", fill="white")
```
The distribution shows that there are extreme outliers. Therefore, we truncate them
```{r}
